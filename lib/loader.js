...
// dep loaders
const styleLoaderPath = normalize.dep('kdu-style-loader')
const hotReloadAPIPath = normalize.dep('kdu-reload-api')
...
// When extracting parts from the source kdu file, we want to apply the
// loaders chained before kdu-loader, but exclude some loaders that simply
...
module.exports = function (content) {
  ...
  Object.defineProperty(this.options, '__kduOptions__', {
    value: options,
    enumerable: false,
    configurable: true
  })
  ...
  if (parts.styles.length) {
    ...
    parts.styles.forEach((style, i) => {
      ...
      const hasKduStyleLoader = requireString.indexOf('kdu-style-loader') > -1
      // kdu-style-loader exposes inject functions during SSR so they are
      // always called
      const invokeStyle =
        isServer && hasKduStyleLoader
      ...
      if (moduleName) {
        ...
        if (moduleName in cssModules) {
          ...
          // `(kdu-)style-loader` exposes the name-to-hash map directly
  ...
  if (script) {
    ...
    if (query.inject) {
      output += '__kdu_script__ = __kdu_script__(injections)\n'
    }
  } else {
    output += 'var __kdu_script__ = null\n'
  }
  ...
  if (template) {
    ...
    output += 'var __kdu_template__ = null\n'
  }

  // template functional
  output += '/* template functional */\n'
  output +=
    'var __kdu_template_functional__ = ' +
  ...
  output +=
    ...
    '  __kdu_script__,\n' +
    '  __kdu_template__,\n' +
    '  __kdu_template_functional__,\n' +
    '  __kdu_styles__,\n' +
    '  __kdu_scopeId__,\n' +
    '  __kdu_module_identifier__\n' +
    ')\n'
  ...
  if (!isProduction) {
    ...
    output +=
      ...
      '})) {' +
      '  console.error("named exports are not supported in *.kdu files.")' +
      '}\n'
  }
  ...
  if (!query.inject) {
    ...
    if (needsHotReload) {
      output +=
        ...
        '  hotAPI.install(require("kdu"), false)\n' +
  ...
  function getRequireString (type, part, index, scoped) {
    return loaderUtils.stringifyRequest(
      ...
      '!!' +
        ...
        // select the corresponding part from the kdu file
        getSelectorString(type, index || 0) +
        // the url to the actual kdu file, including remaining requests
        rawRequest
    )
  }
  ...
  function addCssModulesToLoader (loader, part, index) {
    ...
    return loader.replace(/((?:^|!)css(?:-loader)?)(\?[^!]*)?/, (m, $1, $2) => {
      ...
      if (index !== -1) {
        ...
        //   Different <style> tags in the same .kdu file may generate same names.
  ...
  function getRawLoaderString (type, part, index, scoped) {
    ...
    if (type === 'styles') {
      ...
      styleCompiler =
        ...
        '?' +
        JSON.stringify({
          // a marker for kdu-style-loader to know that this is an import from a kdu file
          kdu: true,
          id: moduleId,
          scoped: !!scoped,
  ...
  function getCSSExtractLoader (lang) {
    ...
    if (typeof op.extract === 'function') {
      ...
      if (!extractor) {
        throw new Error(
          '[kdu-loader] extractCSS: true requires extract-text-webpack-plugin ' +

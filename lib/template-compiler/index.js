...
const transpile = require('kdu-template-es2015-compiler')
...
module.exports = function (html) {
  ...
  const kduOptions = this.options.__kduOptions__ || {}
  const options = loaderUtils.getOptions(this) || {}
  const needsHotReload = !isServer && !isProduction && kduOptions.hotReload !== false
  const defaultModules = [transformRequire(options.transformToRequire), transformSrcset()]
  let userModules = kduOptions.compilerModules || options.compilerModules
  ...
  const compilerOptions = {
    ...
    directives:
      kduOptions.compilerDirectives || options.compilerDirectives || {},
    scopeId: options.hasScoped ? options.id : null,
    comments: options.hasComment
  }

  const compile =
    isServer && compiler.ssrCompile && kduOptions.optimizeSSR !== false
      ? compiler.ssrCompile
      : compiler.compile
  ...
  if (compiled.errors && compiled.errors.length) {
    ...
    // mark with stripped (this enables Kdu to use correct runtime proxy detection)
  ...
  // hot-reload
  if (needsHotReload) {
    const exportsName = kduOptions.esModule ? 'esExports' : 'module.exports'
    code +=
      '\nif (module.hot) {\n' +
      '  module.hot.accept()\n' +
      '  if (module.hot.data) {\n' +

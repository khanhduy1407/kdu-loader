...
For Webpack 1.x: add a root `kdu` block in your Webpack config.
...
- type: `{ [lang: string]: string }`
  ...
  An object specifying Webpack loaders to overwrite the default loaders used for language blocks inside `*.kdu` files. The key corresponds to the `lang` attribute for language blocks, if specified. The default `lang` for each type is:
...
- only supported in 10.3.0+
  ...
  - For `css`, the result will be returned by `kdu-style-loader` which isn't particularly useful in most cases. Using a postcss plugin will be a better option.
...
- type: `Array` or `Function` or `Object`
  ...
  Specify custom PostCSS plugins to be applied to CSS inside `*.kdu` files. If using a function, the function will called using the same loader context and should return an Array of plugins.
...
- default: `true` (v13.0+)
  ...
  Whether to emit esModule compatible code. By default kdu-loader will emit default export in commonjs format like `module.exports = ....`. When `esModule` is set to true, default export will be transpiled into `exports.__esModule = true; exports = ...`. Useful for interoperating with transpiler other than Babel, like TypeScript.
...
- default: `[]`
  ...
  Configure `modules` options for `kdu-template-compiler`. In about details, see more [`modules` option](https://github.com/khanhduy1407/kdu/blob/dev/packages/kdu-template-compiler/README.md#compilercompiletemplate-options) of `kdu-template-compiler`.
...
- default: `{}` (v13.0.5+)
  ...
  Configure `directives` options for `kdu-template-compiler`, In about details, see more [`directives` option](https://github.com/khanhduy1407/kdu/blob/dev/packages/kdu-template-compiler/README.md#compilercompiletemplate-options) of `kdu-template-compiler`.
